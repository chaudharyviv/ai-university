...
class PersonaEssentials(BaseModel):
    """
    Reduced persona output for the hybrid fixed-archetype path (see
    models/persona_archetypes.py). audience_level and tone are hardcoded
    per archetype and never generated by the LLM - only these two
    topic-dependent fields are. PersonaAgent composes this with the
    archetype's fixed fields into a full PersonaProfile.
    """
